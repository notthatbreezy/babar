...
- **FR-002a**: A "Prerequisites" page precedes the first-query chapter and gives the reader a single foreground `docker run` (postgres:17, `--rm -it`, `log_statement=all`, `log_min_duration_statement=0`, `log_connections=on`, `log_disconnections=on`, user/password/db all `babar`) so every tutorial in the book has a Postgres that prints exactly what babar sent on the wire. The landing page Quick Start CTAs link to it before the first-query page. (Stories: P1)
...
- **FR-006a**: The Explanation section also contains a `What makes babar babar` page that gives a one-page tour: where babar sits in the stack, the four distinctive properties (background-driver-task, typestate-at-the-boundary, explicit codecs, validate-early), what babar deliberately is **not**, and when to reach for it. The landing page links this page first under "Where to go next". (Stories: P4)
...
- **SC-006a**: `docs/explanation/what-makes-babar-babar.md` exists, has at least 80 non-blank lines of prose, and contains sections covering each of the four distinctive properties (background-driver-task, typestate, explicit codecs, validate-early), a "what babar is not" list, and a "when babar is the right pick" list. *(FR-006a)*
