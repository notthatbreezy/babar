...
The chosen user-facing direction is a schema-scoped wrapper rather than trying to teach the global `babar::typed_query!` proc macro to inspect arbitrary foreign Rust items directly. The planning assumption is that a schema declaration macro can generate both reusable Rust-visible schema symbols and a local `typed_query!` wrapper that feeds the existing typed-query compilation path with synthesized schema metadata. This keeps the user API Rust-native, supports schemas that contain multiple tables, and minimizes the amount of churn required in the existing resolver/lowering internals.

## Desired End State

Users can declare a reusable schema module containing multiple table declarations through a Rust-visible surface, including field types, nullability, and a narrow set of important semantic markers such as primary-key identity. That schema module exposes a local `typed_query!` wrapper so users can write schema-aware queries without repeating inline schema blocks, while authored and generated schema modules share the same consumption pattern.

Implementation success means the schema declaration surface produces the schema facts required by the current typed-query pipeline, the schema-scoped wrapper compiles queries through the existing SQL-first flow, diagnostics clearly cover authored/generated/external-schema misuse, and docs/examples teach the recommended hybrid pattern. Verification includes targeted UI/runtime/schema tests during each phase and repo-wide lint/test/docs validation before completion.
...
- Shipping a babar-managed schema generator in this milestone
...
- [ ] **Phase 3: Diagnostics and Coverage** - Expand UI, runtime, and schema-level coverage for authored/generated external schema usage and failure modes.
...
- [ ] Add babar-managed schema generation or database-introspection workflows
...
- **`crates/core/src/schema.rs`**: Extend or align the existing Rust-visible schema primitives so generated schema modules can expose reusable table/column/type symbols through the core schema layer.
...
- **Tests**: Add schema-surface unit coverage around generated table/column/type symbols and multi-table schema declarations in the most direct macro/schema test locations.
...
- [ ] The generated/authored schema surface reads as Rust-native and keeps the common field case visibly type-first.
...
### Changes Required:
- **`crates/macros/src/typed_sql/public_schema.rs`**: Refactor the typed-query input path so schema metadata can come from generated schema-module expansion as well as the current inline path.
- **`crates/macros/src/lib.rs`** and **schema declaration expansion code**: Generate a schema-scoped `typed_query!` wrapper that forwards synthesized schema metadata into the existing typed-query entry pipeline.
- **`crates/macros/src/typed_sql/resolver.rs`**: Keep the catalog boundary stable while accepting schema facts bridged from generated schema-module output.
...
### Changes Required:
- **`crates/core/tests/typed_query_ui.rs`** and **`crates/core/tests/ui/typed_query/**`**: Add pass/fail coverage for external schema references, authored/generated parity, missing tables or columns, unsupported metadata markers, and any rejected mixed inline/external usage combinations.
...
- **`crates/core/src/schema.rs`** test fixtures: Expand fixture-style schema modules to cover reusable symbols, marker semantics, and multi-table declarations.
- **`crates/macros/src/typed_sql/public_schema.rs`** and **`resolver.rs`**: Keep diagnostics specific for external-schema misuse and catalog-resolution failures.
...
- [ ] Docs consistently explain the v1 limits: Rust-visible schemas only, SQL-first queries, and no file-based schema inputs.
