...
**Input Brief**: Add ergonomic external schema support so `typed_query!` can use authored Rust-visible schema declarations instead of requiring inline schema blocks.

## Overview

`typed_query!` already proves that babar can validate read queries against declared schema information, but the current inline schema block is still a local proof-of-concept authoring model. It works for small examples, yet it becomes repetitive and awkward once a project wants to reuse the same table definitions across queries and modules. The next step is to let users define schema information once and reference it from typed queries without falling back to stringly or file-path-heavy configuration.
...
This feature therefore focuses on a Rust-visible external schema model for authored declarations. Users should be able to keep schema facts in reusable Rust-facing artifacts, use them across many queries, and preserve babar's SQL-first query surface. In v1, the chosen query entrypoint is a schema-scoped wrapper such as `app_schema::typed_query!(...)`, so one schema module can provide the shared context for multiple queries without requiring the global `babar::typed_query!` callsite to inspect arbitrary Rust items directly. The user experience should stay simple enough that external schemas feel like a convenience and ergonomics upgrade, not like a second framework layered on top of `typed_query!`.
...
- Support authored Rust-visible schema declarations that can be reused across many queries and tables.
...
- FR-002: The first version of external schemas must target Rust-visible authored declarations as the supported source model. (Stories: P1)
...
- FR-004: The external schema experience must preserve the existing SQL-first typed query model so users continue authoring queries as SQL rather than through a second query builder surface, with a schema-scoped `typed_query!` wrapper as the chosen v1 entrypoint. (Stories: P1)
...
- FR-007: One authored schema module must be usable as shared query context for multiple typed queries and multiple declared tables without repeating schema definitions at each callsite. (Stories: P1)
...
- Schema Module: A Rust-visible declaration surface that can contain multiple table definitions and act as the context for schema-scoped typed queries.
...
- The recommended pattern should remain understandable when declarations are authored by hand across multiple modules and tables.
...
3. **Hybrid pattern**: users define a Rust-visible schema module, but field declarations can use reusable semantic wrappers so the common case stays terse while richer semantics remain available when needed. In v1 this hybrid pattern is exposed through a schema-scoped query wrapper, so the schema module itself becomes the query context for multiple tables.

The recommended direction for v1 is the **hybrid pattern**, implemented as a multi-table schema module plus a schema-scoped `typed_query!` wrapper, because it keeps the schema source explicit and reusable while still allowing the field model itself to feel Rust-native and type-driven.
...
- SC-003: Users can employ one authored schema module as shared context for multiple queries and multiple declared tables through the same schema-scoped query flow. (FR-002, FR-007)
- SC-004: The external schema feature preserves babar's SQL-first query experience rather than introducing a separate builder-centric query model, while the schema-scoped wrapper remains the explicit v1 query entrypoint. (FR-004)
...
- Schema code generation or generated schema artifacts in v1
...
- Users may expect schema code generation to be part of “external schemas”: Mitigation: state explicitly that v1 is authored-schema-only and defer code generation entirely.
