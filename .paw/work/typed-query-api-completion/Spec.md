...
This effort therefore focuses on making the typed query API feel complete for read paths: schema-aware `SELECT`s, explicit optional predicate/group boundaries, optional ordering and pagination clauses, and clear rules for how absent inputs change the emitted query. The optional syntax should stay visually close to SQL by using suffix markers such as `$id?` for optional single inputs and `(...)?` for optional owned groups or clauses. The result should feel like SQL with a small amount of structure-aware optionality, not like a general templating language or ORM DSL.
...
- Preserve a SQL-first reading experience, with only small explicit suffix markers for optional behavior.
...
Narrative: A babar user defines a listing query with several optional filters and expects only the active filters to contribute to the SQL sent to the server, using SQL-like forms such as `$id?`.
...
Narrative: A babar user writes a compound condition and needs grouped optional logic to disappear only at declared group boundaries such as `(...)?` so the query meaning does not change unexpectedly.
...
- A suffix-style optional marker appears in a context where ownership of the omitted SQL is ambiguous.
...
- FR-003: The API must let query authors declare optional single-filter inputs for read queries using a lightweight SQL-adjacent form such as `$id?` so inactive filters are omitted from emitted SQL. (Stories: P1)
- FR-004: The API must let query authors declare explicit optional predicate/group boundaries using a lightweight SQL-adjacent form such as `(...)?` so compound conditions are omitted only at clear author-controlled boundaries. (Stories: P1)
...
- Optional Filter Input: A declared read-query input, represented by syntax such as `$id?`, whose absence removes an owned predicate from the emitted SQL.
- Optional Group Boundary: An explicit author-controlled grouping, represented by syntax such as `(...)?`, that causes an entire predicate/group fragment to be omitted when its required optional inputs are not all present.
...
- Optional syntax should remain visually close to normal SQL and avoid introducing a large secondary DSL.
...
- SC-003: Grouped optional predicates written with the supported group syntax are omitted as whole units when required grouped inputs are missing, with no partial-condition rewrites in supported cases. (FR-004, FR-007)
...
- Small suffix-style optional markers such as `$id?` and `(...)?` are acceptable to users so long as the surrounding query remains recognizably SQL.
...
- Suffix-style optional syntax for single inputs and owned groups/clauses
...
- Omission rules may become confusing if too many cases are supported at once: Mitigation: keep v1 narrow, require explicit suffix-style optional boundaries, and reject unsupported placements.
...
- Suffix syntax may introduce parsing ambiguities around legal ownership boundaries: Mitigation: explicitly define where `$value?` and `(...)?` are valid and reject ambiguous placements with diagnostics.
