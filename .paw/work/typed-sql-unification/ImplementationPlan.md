...
path, expanding that compiler beyond its current `SELECT`-only boundary, and
building live verification and broader runtime type support into the unified
surface.
...
- Authored schema modules already integrate cleanly through `schema!` and a
  ...
  seam for public macro unification
...
2. `command!` is the default schema-aware typed surface for supported write
   statements, including explicit `RETURNING` forms that lower through the same
   row-shape machinery as `SELECT`.
3. The unified typed SQL compiler supports the agreed first DML subset and the
   ...
   columns.
4. Optional live verification checks authored schema facts and statement
   metadata against a live database through the typed SQL path.
...
- [ ] **Phase 2: Typed DML Support** - Add the agreed `INSERT` / `UPDATE` / `DELETE` subset, including explicit `RETURNING`, and connect it to authored schema modules.
...
- **`crates/macros/src/typed_sql/ir.rs`**: Introduce statement-shape-aware IR that can represent row-returning and command-style statements without forking the compiler into unrelated stacks.
...
### Changes Required:
- **`crates/macros/src/typed_sql/parse_backend.rs`**: Expand parsing beyond the current single-`SELECT` gate to cover the agreed `INSERT ... VALUES`, `UPDATE ... SET ... WHERE`, and `DELETE ... WHERE` subset, with explicit support for explicit-column `RETURNING`.
- **`crates/macros/src/typed_sql/normalize.rs`**: Normalize the agreed DML subset into typed SQL IR while continuing to reject out-of-scope constructs such as `ON CONFLICT`, subqueries, `WITH`, `UPDATE ... FROM`, `DELETE ... USING`, and wildcard `RETURNING *`.
- **`crates/macros/src/typed_sql/resolver.rs`**: Add statement-kind-aware schema resolution for target tables, assignments, predicates, and `RETURNING` projections; reuse the existing row/projection machinery for explicit `RETURNING`.
- **`crates/macros/src/schema_decl.rs`**: Add schema-scoped command-side bridge support so authored schema modules expose the same default ergonomics for supported write statements as they do for read statements.
- **Tests**:
  - new UI fixtures under `crates/core/tests/ui/typed_query/**` or parallel typed-command coverage for supported/unsupported DML
...
#### Manual Verification:
- [ ] Supported `INSERT`, `UPDATE`, and `DELETE` statements feel like the same schema-aware typed SQL family as read queries.
- [ ] Unsupported DML constructs fail with subset diagnostics instead of falling through into confusing parser/runtime errors.
...
- **`crates/macros/src/typed_sql/public_schema.rs`**: Add typed-SQL verification orchestration so authored schema facts, referenced tables/columns, parameters, and returned/`RETURNING` columns can be checked against a live database.
...
### Changes Required:
- **`crates/macros/src/lib.rs`**: Repoint public `query!` / `command!` macro entrypoints onto the unified schema-aware typed SQL compiler, while staging compatibility behavior so the surface transition is deliberate rather than ad hoc.
...
#### Manual Verification:
- [ ] A user reading the public macro API sees schema-aware `query!` / `command!` as the default path rather than `typed_query!` or explicit codec tuples.
