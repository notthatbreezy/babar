...
**Branch**: feature/typed-sql-unification  |  **Created**: 2026-04-29  |  **Status**: Draft
...
Narrative: A babar user wants the main read-query API to understand authored schema facts directly, without having to manually declare parameter and row codecs for ordinary supported queries.
Independent Test: Define an authored schema module, write a supported read query with `query!`, and confirm it compiles with inferred typed parameter and row behavior.
...
Independent Test: Write supported insert, update, and delete statements with `command!` and confirm they validate and execute with typed parameter behavior.
Acceptance Scenarios:
1. Given a supported `INSERT`, `UPDATE`, or `DELETE` statement, When a user writes it with `command!`, Then the statement validates against authored schema facts and exposes the correct typed parameter behavior.
2. Given a supported write statement with `RETURNING` explicit columns, When the user compiles it, Then the statement exposes typed row-returning behavior consistent with the declared returned columns.

### User Story P1 – Verify authored schema facts against a live database
Narrative: A babar user wants authored schema modules to remain the primary source of truth in code, while optional live verification confirms those declarations still match the real database.
...
- A user attempts unsupported SQL constructs such as CTEs, subqueries, `ON CONFLICT`, wildcard `RETURNING`, or multi-statement batches.
...
- FR-001: `query!` must become the primary schema-aware typed read-query surface for supported row-returning statements. (Stories: P1)
- FR-002: `command!` must become the primary schema-aware typed write-command surface for supported no-row and explicit-`RETURNING` write statements. (Stories: P1)
- FR-003: The primary typed SQL model must use authored schema modules as the main schema source for supported typed statements. (Stories: P1)
...
- FR-005: The main typed SQL path must support optional live verification that checks authored schema facts, referenced tables and columns, parameters, and returned or `RETURNING` columns against a live database. (Stories: P1)
- FR-006: The supported write subset for this round must include `INSERT ... VALUES ...`, `INSERT ... RETURNING <explicit columns>`, `UPDATE ... SET ... WHERE ...`, `UPDATE ... RETURNING <explicit columns>`, `DELETE ... WHERE ...`, and `DELETE ... RETURNING <explicit columns>`. (Stories: P1)
- FR-007: The typed SQL path for this round must explicitly reject unsupported constructs including `ON CONFLICT`, `INSERT ... SELECT`, `WITH` / CTEs, subqueries, `UPDATE ... FROM`, `DELETE ... USING`, wildcard `RETURNING *`, and multi-statement batches. (Stories: P1,P2)
...
- Typed Write Statement: A supported write SQL statement authored with `command!`, with or without explicit `RETURNING` columns.
...
- Live Verification: Optional compile-time checking that compares authored schema expectations and statement expectations against a real database.
...
- SC-002: Users can author supported insert, update, and delete statements with `command!`, including explicit `RETURNING` variants, without falling back to the old explicit-codec macro model. (FR-002, FR-006)
...
- SC-006: Public docs present schema-aware `query!` / `command!` as the default path, reserve `simple_query_raw` for migrations/control-plane work, and describe raw `Query::raw` / `Command::raw` as advanced escape hatches. (FR-010, FR-011, FR-012, FR-013)
...
- Typed read statements in the supported subset
...
- Replacing the old macro behavior in place may create adoption friction: Mitigation: document the new default clearly, keep explicit raw escape hatches, and define a deliberate migration story.
